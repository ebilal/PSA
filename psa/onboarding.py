...
Usage:
    python3 -m psa.onboarding
    or: psa init
"""

from pathlib import Path
from psa.entity_registry import EntityRegistry
from psa.entity_detector import detect_entities, scan_for_detection
...
def _warn_ambiguous(people: list) -> list:
    ...
    """
    from psa.entity_registry import COMMON_ENGLISH_WORDS
...
def _generate_aaak_bootstrap(
    ...
    """
    psa_dir = Path(config_dir) if config_dir else Path.home() / ".psa"
    ...
    registry_lines = [
        "# AAAK Entity Registry",
        "# Auto-generated by psa init. Update as needed.",
        "",
        "## People",
    ]
    for p in people:
    ...
    (psa_dir / "aaak_entities.md").write_text("\n".join(registry_lines))
    ...
    (psa_dir / "critical_facts.md").write_text("\n".join(facts_lines))
...
""")
    ...
    print("\n  AAAK entity registry: ~/.psa/aaak_entities.md")
    print("  Critical facts bootstrap: ~/.psa/critical_facts.md")
