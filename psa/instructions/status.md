...
Check if MCP tools are available (look for psa_status in available tools).

- If MCP is available: Call the psa_status tool to retrieve palace state.
- If MCP is not available: Run the CLI command: psa status
...
- psa_kg_stats -- for a knowledge graph overview (triple count, entity
  count, relationship types)
- psa_graph_stats -- for connectivity information (connected components,
...
- Empty palace (zero memories): Suggest "Try /psa:mine to add data from
  files, URLs, or text."
- Has data but no knowledge graph (memories exist but KG stats show zero
  ...
  queries."
- Healthy palace (has memories and KG data): Suggest "Use /psa:search to
  query your memories."
