...
## Step 2: Check if psa is already installed

Run `pip show psa` to see if the package is already present. If it is,
...
If `pip install psa` fails, try these fallbacks in order:

1. Try `pip3 install psa`
2. Try `python -m pip install psa` (or `python3 -m pip install psa`)
...
Run `psa init <dir>` where `<dir>` is the directory from Step 4.
...
Run `psa status` and confirm the output shows a healthy palace.
...
- Use /psa:mine to start adding data to their palace
- Use /psa:search to query their palace and retrieve stored knowledge
