...
def _capture_hook_output(hook_fn, data, harness="claude-code", state_dir=None):
    ...
    import io
    buf = io.StringIO()
    patches = [patch("psa.hooks_cli._output", side_effect=lambda d: buf.write(json.dumps(d)))]
    if state_dir:
        patches.append(patch("psa.hooks_cli.STATE_DIR", state_dir))
...
def test_stop_hook_passthrough_when_active(tmp_path):
    with patch("psa.hooks_cli.STATE_DIR", tmp_path):
