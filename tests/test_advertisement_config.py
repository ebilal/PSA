...
def test_defaults(tmp_path):
    ...
    assert ad.removal_enabled is True
    ...
    assert ad.sustained_cycles == 21
    assert ad.min_patterns_floor == 5
...
def test_json_config_controls_advertisement_decay(tmp_path):
    from psa.config import MempalaceConfig
    from psa.advertisement.config import AdvertisementDecayConfig
    ...
    assert ad.grace_days == 45
    assert ad.sustained_cycles == 28
    assert ad.min_patterns_floor == 7
