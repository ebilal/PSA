"""Tests for psa.advertisement.decay — D1 rule + P1 shield + P3 pin."""
...
def test_d1_pattern_too_young_is_not_candidate(tmp_path, monkeypatch):
    ...
    from psa.advertisement.decay import DecayParams, decay_report
    ...
    from psa.advertisement.metadata import PatternMetadata, metadata_key, save_metadata
    ...
    params = DecayParams(
        grace_days=30, decay_window_days=60, low_activation_percentile=0, min_anchor_activations=0
    )
    with patch("psa.advertisement.decay._load_atlas_for_tenant", return_value=atlas):
        with patch("psa.advertisement.decay._now_utc", return_value=NOW):
...
def test_p1_shield_below_absolute_floor(tmp_path, monkeypatch):
    ...
    params = DecayParams(
        grace_days=30, decay_window_days=60, low_activation_percentile=0, min_anchor_activations=10
    )
    with patch("psa.advertisement.decay._load_atlas_for_tenant", return_value=atlas):
        with patch("psa.advertisement.decay._now_utc", return_value=NOW):
            report = decay_report("default", params=params, origins={"interactive"})
