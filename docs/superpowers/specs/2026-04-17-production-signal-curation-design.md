...
Per anchor `A`, the curator builds a pool with two separately-tracked components that carry **distinct** information:

- **`oracle_queries`**: for every oracle label with `A ∈ winning_oracle_set`, include the label's `query`. Normative signal — phrasings the LLM judge said *should* land on `A`.
- **`endorsed_fingerprint_queries`**: fingerprint queries for `A`, included iff `A` has appeared in at least one oracle label's `winning_oracle_set` (anchor-level oracle endorsement). Observed signal — live user phrasings that actually landed on `A`, minimally vetted by the fact that the anchor has been oracle-validated.

Anchor-level endorsement (rather than per-query exact match) is the **correct** MVP rule: fingerprint queries and oracle-label queries come from different populations (live `psa search` vs. queries generated by `psa label` over session text), so per-query exact overlap rarely hits in practice. Anchor-level endorsement keeps fingerprints as a genuinely complementary signal rather than a subset of the oracle set.
...
**Excluded:**
- Fingerprint queries for anchors with **zero** oracle endorsements. Drops the drift risk on anchors the LLM judge has never validated.
- Oracle labels where `A` is not in `winning_oracle_set`.

**Empty-pool behavior:** if neither component has entries, the anchor contributes no new candidate patterns. The anchor's card passes through unchanged. If *every* anchor has an empty pool, `curate` writes nothing at all (see §4 — Empty-run guard).

### Support semantics are provisional

Anchor-level endorsement is a **permissive bootstrap**, not the intended long-term contract. A single oracle endorsement unlocks all fingerprints for that anchor — fine for MVP (most anchors get validated during labeling), but it doesn't guard against a fingerprint query that happens to be off-topic for the anchor even when the anchor is broadly valid.

Metadata records the current semantics in a dedicated field (`support_semantics: "anchor_level_oracle_endorsement"`) so future runs can advertise their own rule without confusing provenance. Reserved values for later:
- `"anchor_level_oracle_endorsement_threshold_N"` — require ≥N oracle endorsements per anchor before its fingerprints qualify.
- `"query_level_exact_match"` — per-query exact text match. (Was originally proposed as MVP; rejected because fingerprint and oracle queries rarely overlap by exact text.)
- `"query_level_semantic_match"` — per-query embedding-cosine or canonicalized match. Stronger guarantee; requires an embedding comparison pass at curation time.

This field prevents "whatever the MVP rule was" from drifting into doctrine.
...
If a curation run produces no changes — either because no anchor has oracle support (e.g., `psa label` has never run for this tenant) or because the extractor adds only duplicates of existing patterns — `curate` writes **nothing** to disk. No candidate JSON, no meta JSON. It prints a warning that names the cause and exits with status 0.

This matches Branch 1's `refine` behavior on an empty miss log. It also prevents a footgun: an operator running `curate` on a tenant that already has an unpromoted miss-log candidate would otherwise clobber that candidate with an empty one.

The write-guard condition, evaluated after pool construction and extraction:

- **Skip write** if `n_anchors_touched == 0` OR `n_patterns_added == 0`.
- Otherwise, write both candidate JSON and meta JSON.
...
- Oracle labels file missing → soft: log a warning, run with empty oracle set. The downstream empty-run guard (§4) prevents clobbering any in-flight candidate; if there's no signal, no write happens.
- Fingerprint store missing → same soft behavior. Signal absence is not failure.
- Zero anchors touched or zero patterns added (for any reason) → print a diagnostic warning naming the cause, exit 0 without writing.
...
| `tests/test_curation_pool.py` | Tests for `build_pool`: anchor-level oracle-endorsement filter, empty-pool (no oracle support), both-components-populated, oracle_queries preserved separately from endorsed_fingerprint_queries. |
...
- `psa atlas curate` produces a candidate (JSON + meta) with correct schema, including the provisional `support_semantics` marker and extractor name, **only when the run produces real changes**.
- A tenant with no oracle labels produces **no** candidate file; the command exits 0 with a warning that names the cause. An in-flight miss-log candidate is not clobbered.
- A tenant with oracle labels but no new patterns (all extractor output duplicated existing patterns) triggers the same skip-write path.
- Fingerprint queries contribute to the pool for any anchor that has been oracle-endorsed at least once (anchor-level rule); fingerprints for anchors with zero oracle endorsements are dropped.
